...
class AuthService {
  ...
  async loginWithEmailPassword(email, password) {
    ...
    try {
      ...
      this._credentialSession = { accessToken, idToken, expiresAt, username }
      localStorage.setItem('credential_session', JSON.stringify(this._credentialSession))

      // no constructor, restaura se existir
      const saved = localStorage.getItem('credential_session')
      if (saved) {
        const parsed = JSON.parse(saved)
        if (parsed.expiresAt > Date.now()) {
          // só restaura se não expirou
          this._credentialSession = parsed
        } else {
          localStorage.removeItem('credential_session') // limpa se expirado
        }
      }

      // no logout, limpa
      localStorage.removeItem('credential_session')
